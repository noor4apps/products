                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.users.store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-floating mb-3">
                                <input name="first_name" class="form-control" id="first_name" type="text" value="{{ old('first_name') }}" placeholder="First Name" />
                                <label for="first_name">First Name</label>
                            </div>
                            @error('first_name') <p class="fst-italic text-danger"> {{ $message }} </p> @enderror

                            <div class="form-floating mb-3">
                                <input name="last_name" class="form-control" id="last_name" type="text" value="{{ old('last_name') }}" placeholder="Last Name" />
                                <label for="last_name">Last Name</label>
                            </div>
                            @error('last_name') <p class="fst-italic text-danger"> {{ $message }} </p> @enderror

                            <div class="form-floating mb-3">
                                <input name="email" class="form-control" id="email" type="email" value="{{ old('email') }}" placeholder="Email" />
                                <label for="email">Email</label>
                            </div>
                            @error('email') <p class="fst-italic text-danger"> {{ $message }} </p> @enderror

                            <div class="form-floating mb-3">
                                <input name="phone_number" class="form-control" id="phone_number" type="text" value="{{ old('phone_number') }}" placeholder="Phone Number" />
