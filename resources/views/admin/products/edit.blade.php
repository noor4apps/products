                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-floating mb-3">
                                <input name="name" class="form-control" id="name" type="text" value="{{ old('name', $product->name) }}" placeholder="Name" />
                                <label for="name">Name</label>
                            </div>
                            @error('name') <p class="fst-italic text-danger"> {{ $message }} </p> @enderror

                            <div class="form-floating mb-3">
                                <input name="description" class="form-control" id="description" type="text" value="{{ old('description', $product->description) }}" placeholder="Description" />
                                <label for="description">Description</label>
                            </div>
                            @error('description') <p class="fst-italic text-danger"> {{ $message }} </p> @enderror

                            <div class="form-floating mb-3">
                                <input name="image" class="form-control" type="file" value="{{ old('image', $product->image) }}" />
                            </div>
